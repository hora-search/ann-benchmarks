...
def run_docker(definition, dataset, count, runs, timeout, batch, cpu_limit,
    ...
    try:
        ...
        # Exit if exit code
        if exit_code not in [0, None] and not isinstance(exit_code, dict):
            logger.error(colors.color(container.logs().decode(), fg='red'))
            logger.error('Child process for container {} raised exception {}'.format(container.short_id, exit_code))
    except:
